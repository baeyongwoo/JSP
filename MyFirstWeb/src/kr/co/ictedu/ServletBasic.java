...
public class ServletBasic extends HttpServlet {
	 ...
	 */
    // HttpServlet 객체를 사용받은 클래스 내라고 해서
    // 무턱대고 만든 모든 메서드가 주소 접속시마다 호출되지는 않는다
    // 아래와 같이 상속받은 메서드를 오버라이딩 하는 경우만 
    // 호출이 됨
	public void init(ServletConfig config) throws ServletException {
		// 요청이 들어왔을 때 처음에 실행할  로직을 작성하는 부분
		// init()메서드는 서블릿 객체 생성시 1번 자동으로 호출
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// HTTP Post 요청에 들어왔을때만 작동하는 메서드
		// .jsp파일에 폼을 만들고, post 요청을 하도록 처리
